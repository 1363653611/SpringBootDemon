 ...
 */
require(['fileUpload',"jquery"], function(fileUpload,$){
　　　　// some code here
	/**
	 * 单文件上传监听
	 */
	fileUpload.singleUploadForm.addEventListener('submit', function(event){
		var files = fileUpload.singleFileUploadInput.files;
		if(files.length === 0) {
			fileUpload.singleFileUploadError.innerHTML = "Please select a file";
			fileUpload.singleFileUploadError.style.display = "block";
		}
		fileUpload.uploadSingleFile(files[0]);
	 ...
	 * 多文件上传按钮监听
	 */
	fileUpload.multipleUploadForm.addEventListener('submit', function(event){
		var files = fileUpload.multipleFileUploadInput.files;
		if(files.length === 0) {
			fileUpload.multipleFileUploadError.innerHTML = "Please select at least one file";
			fileUpload.multipleFileUploadError.style.display = "block";
		}
		fileUpload.uploadMultipleFiles(files);
		event.preventDefault();
	}, true);
})